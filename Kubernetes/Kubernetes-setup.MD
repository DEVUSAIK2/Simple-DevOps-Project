...
1. install AWSCLI
   ```sh
    ...
    apt install unzip python     if it throws error then $sudo apt-get update
...
1. Install kubectl on ubuntu instance
   ```sh
   KubeCtl is a tool which is used to manage our Kubernetes Clusters
...
1. Install kops on ubuntu instance
   ```sh
    Kops is a tool useful to setup Kubernetes clusters on AWS
